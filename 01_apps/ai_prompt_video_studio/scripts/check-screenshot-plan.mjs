...
if (plan) {
  ...
  for (const set of sets) {
    ...
    for (const shot of shots) {
        ...
        if (!Array.isArray(shot.interactions)) {
          ...
          for (const interaction of shot.interactions) {
            ...
            if (interaction.type === "clickText") {
              if (!String(interaction.text || "").trim()) {
              ...
              if (!String(interaction.selector || "").trim()) {
                block(`${id}/${shot.id || "unknown"} clickSelector interaction is missing selector.`);
              } else {
                pass(`${id}/${shot.id || "unknown"} has a valid clickSelector interaction.`);
              }
            } else if (interaction.type === "scrollToSelector") {
              if (!String(interaction.selector || "").trim()) {
                block(`${id}/${shot.id || "unknown"} scrollToSelector interaction is missing selector.`);
              } else {
                pass(`${id}/${shot.id || "unknown"} has a valid scrollToSelector interaction.`);
              }
            } else {
              block(`${id}/${shot.id || "unknown"} interaction type must be clickText, clickSelector, or scrollToSelector.`);
            }
          }
        }
      }
      ...
      if (shot.id === "home-notifications") {
        const hasNotificationClick = Array.isArray(shot.interactions)
          && shot.interactions.some((interaction) => interaction.type === "clickSelector" && interaction.selector === ".notification-button");
        if (hasNotificationClick) {
          pass(`${id}/home-notifications captures the opened notification panel.`);
        } else {
          block(`${id}/home-notifications must click .notification-button before capture.`);
        }
      }
      if (shot.id === "pwa-install-guide") {
        if (shot.showPwaInstallGuide === true) {
          pass(`${id}/pwa-install-guide keeps the PWA install guide visible for capture.`);
        } else {
          block(`${id}/pwa-install-guide must set showPwaInstallGuide=true.`);
        }
        const scrollsToInstallGuide = Array.isArray(shot.interactions)
          && shot.interactions.some((interaction) => interaction.type === "scrollToSelector" && interaction.selector === ".pwa-install-banner");
        if (!scrollsToInstallGuide) {
          pass(`${id}/pwa-install-guide captures the install guidance card in the first viewport.`);
        } else {
          block(`${id}/pwa-install-guide must be visible without scrolling.`);
        }
      }
    }
