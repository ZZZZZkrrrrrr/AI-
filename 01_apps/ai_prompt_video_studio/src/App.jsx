...
const navItems = [
  ...
  },
  {
    ...
    children: [
      ...
      { id: "personAssets", label: "人物素材库", icon: ShieldCheck },
...
const mobileTabItems = [
  ...
  { id: "assets", label: "素材", icon: FolderOpen, activePages: ["assets", "tasks", "libtv", "personAssets"] },
...
export function App() {
  ...
  const [iosInstallGuideVisible, setIosInstallGuideVisible] = useState(() => {
    try {
      return localStorage.getItem(pwaInstallDismissedKey) !== "true" && isIosInstallCandidate();
    } catch {
      return false;
    }
  });
  const showNativeInstallPrompt = Boolean(pwaInstallPrompt && !pwaInstallDismissed);
  const showIosInstallGuide = Boolean(iosInstallGuideVisible && !showNativeInstallPrompt && !pwaInstallDismissed);
  ...
  useEffect(() => {
    ...
    setIosInstallGuideVisible(isIosInstallCandidate());
  ...
  async function loadPersonAssets() {
    try {
      const data = await requestJson("/api/person-assets");
      setPersonAssets(Array.isArray(data.assets) ? data.assets : []);
    } catch (error) {
      setPersonAssets([]);
      addNotification({
        level: "warn",
  ...
  async function savePersonAsset(assetPayload) {
    const hasId = Boolean(assetPayload?.id);
    const path = hasId ? `/api/person-assets/${encodeURIComponent(assetPayload.id)}` : "/api/person-assets";
    const data = await requestJson(path, {
      method: "POST",
      body: JSON.stringify(assetPayload)
    });
    setPersonAssets((current) => [data.asset, ...current.filter((item) => item.id !== data.asset.id)]);
    addNotification({
      level: data.asset?.complianceStatus === "approved" ? "success" : "info",
      title: "人物素材已保存",
      message: `${data.asset?.name || "人物素材"}：${data.asset?.complianceStatusText || "待校验"}`,
      target: "personAssets"
    });
    return data.asset;
  }
  ...
  const currentPage = useMemo(() => {
    ...
    if (page === "personAssets") {
      return <PersonAssetsPage assets={personAssets} onRefresh={loadPersonAssets} onSave={savePersonAsset} navigate={navigate} />;
    }
  ...
  }, [page, tasks, jobs, batchJobs, batchDetail, batchFocusId, assets, assetTaskCode, runtime, selectionAssets, personAssets, studio, promptSteps, promptRunning, runId, videoRunning, videoSteps, videoLog, modelSettings, modelTrace]);
...
function BatchPage({ runtime, modelSettings, personAssets = [], batchJobs, batchDetail, focusBatchId = "", loadBatchJobs, loadBatchDetail, addNotification, onBatchFocusHandled, onRefreshAll }) {
  ...
  const approvedPersonAssets = useMemo(
    () => (personAssets || []).filter((asset) => asset.complianceStatus === "approved"),
    [personAssets]
  );
  const selectedPersonAsset = approvedPersonAssets.find((asset) => asset.id === selectedPersonAssetId) || null;

  useEffect(() => {
    if (selectedPersonAssetId && !approvedPersonAssets.some((asset) => asset.id === selectedPersonAssetId)) {
      setSelectedPersonAssetId("");
    }
  }, [approvedPersonAssets, selectedPersonAssetId]);
  ...
  function restoreBatchDraft(draft) {
    ...
    setSelectedPersonAssetId(draft.selectedPersonAssetId || "");
      ...
      const data = await requestJson("/api/batches", {
        ...
        body: JSON.stringify({
          ...
          items: selectedRows.map((row) => ({
            ...
            personAssetId: selectedPersonAsset?.id || "",
            modelSettings: buildModelSettingsPayload(modelSettings),
            maxRetries: 1
          }))
        })
      ...
      <div className="batch-layout">
        <div className="panel batch-builder">
          ...
          <div className="batch-config-grid">
            ...
            <label className="field batch-person-asset-field">
              <span>复用人物素材</span>
              <select value={selectedPersonAssetId} onChange={(event) => setSelectedPersonAssetId(event.target.value)}>
                <option value="">不使用人物素材</option>
                {approvedPersonAssets.map((asset) => (
                  <option value={asset.id} key={asset.id}>{asset.name} · {asset.complianceStatusText || "已通过"}</option>
                ))}
              </select>
              {!approvedPersonAssets.length ? <small>暂无已通过人物素材，可先到人物素材库登记。</small> : null}
            </label>
          </div>
          {selectedPersonAsset ? (
            <div className="batch-person-asset-note">
              <ShieldCheck size={16} />
              <span>本批次会复用 <strong>{selectedPersonAsset.name}</strong>，系统会把这张已合规人物图追加为每条任务的人物参考图。</span>
            </div>
          ) : null}
...
const personAssetStatusOptions = [
  { value: "pending", label: "待校验" },
  { value: "manual_required", label: "需人工校验" },
...
function emptyPersonAssetDraft() {
  return {
    name: "",
    personType: "real_person",
    complianceStatus: "pending",
    complianceMethod: "manual",
    libtvProjectId: "",
    libtvImageNodeKey: "",
    libtvImageNodeName: "",
    assetId: "",
...
function personAssetToDraft(asset = {}) {
  return {
    ...emptyPersonAssetDraft(),
    name: asset.name || "",
    personType: asset.personType || "real_person",
    complianceStatus: asset.complianceStatus || "pending",
    complianceMethod: asset.complianceMethod || "manual",
    libtvProjectId: asset.libtvProjectId || "",
    libtvImageNodeKey: asset.libtvImageNodeKey || "",
    libtvImageNodeName: asset.libtvImageNodeName || "",
...
function personAssetStatusTone(status) {
...
function personAssetTypeText(value) {
  if (value === "digital_human") return "数字人";
  if (value === "model_reference") return "模特参考";
  return "真人图片";
}

function PersonAssetsPage({ assets = [], onRefresh, onSave, navigate }) {
  const [selectedId, setSelectedId] = useState(() => assets[0]?.id || "new");
  const [creatingNew, setCreatingNew] = useState(false);
  const selectedAsset = assets.find((asset) => asset.id === selectedId);
  const [draft, setDraft] = useState(() => personAssetToDraft(selectedAsset || {}));
  const [imageFile, setImageFile] = useState(null);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState("");
  const approvedAssets = assets.filter((asset) => asset.complianceStatus === "approved");
  const pendingAssets = assets.filter((asset) => !["approved", "rejected", "expired"].includes(asset.complianceStatus));
  const rejectedAssets = assets.filter((asset) => ["rejected", "expired"].includes(asset.complianceStatus));
  const previewImage = imageFile?.dataUrl || selectedAsset?.imageUrl || "";

  useEffect(() => {
    if (!creatingNew && selectedId === "new" && assets.length) {
      setSelectedId(assets[0].id);
    }
  }, [assets, creatingNew, selectedId]);

  useEffect(() => {
    if (selectedId === "new") {
      setDraft(emptyPersonAssetDraft());
      setImageFile(null);
      return;
    }
    if (selectedAsset) {
      setDraft(personAssetToDraft(selectedAsset));
      setImageFile(null);
    }
  }, [selectedId, selectedAsset?.id, selectedAsset?.updatedAt]);

  useEffect(() => {
    if (selectedId !== "new" && !assets.some((asset) => asset.id === selectedId)) {
      setCreatingNew(false);
      setSelectedId(assets[0]?.id || "new");
    }
  }, [assets, selectedId]);
  ...
  async function handleImageChange(files) {
    ...
    setImageFile(parsed);
    if (!draft.name) updateDraft("name", fileBase(file.name));
  }

  async function submitPersonAsset(event) {
    event.preventDefault();
    ...
    if (!imageFile && !selectedAsset?.imageUrl) {
      alert("请先上传一张人物图片。");
      return;
    }
    try {
      ...
      setDraft(personAssetToDraft(saved));
      setImageFile(null);
      setMessage(`${saved.name} 已保存，状态：${saved.complianceStatusText || "待校验"}`);
    } catch (error) {
      alert(error.message || "人物素材保存失败");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <section className="selection-page person-assets-page">
      <div className="panel selection-hero">
        <div>
          <h2>人物素材库</h2>
          <p>把已经完成合规校验的人物图片保存下来。批量生成时可以直接复用，系统会把它追加为人物参考图，减少真人图重复失败。</p>
        </div>
        <div className="hero-actions">
          <button className="secondary-button" type="button" onClick={onRefresh}>
            <RefreshCw size={16} />
          ...
          <button className="primary-button" type="button" onClick={() => navigate("batch")}>
            <ListChecks size={16} />
            <span>去批量生成</span>
          </button>
        </div>
      ...
      <div className="person-asset-summary">
        <div><span>总素材</span><strong>{assets.length}</strong></div>
        <div><span>已通过</span><strong>{approvedAssets.length}</strong></div>
        <div><span>待处理</span><strong>{pendingAssets.length}</strong></div>
        <div><span>不可用</span><strong>{rejectedAssets.length}</strong></div>
      </div>

      {message ? <p className="selection-task-message">{message}</p> : null}

      <div className="person-assets-workbench">
        <div className="panel person-asset-library-panel">
          <div className="panel-head">
            <div>
              <h2>素材列表</h2>
              <p className="panel-note">只会在批量生成里显示“已通过”的素材。</p>
            </div>
            <button className="secondary-button person-asset-new-button" type="button" onClick={() => {
              setCreatingNew(true);
              setSelectedId("new");
            }}>
              <Upload size={16} />
          ...
          <div className="person-asset-picker-list">
            {assets.length ? assets.map((asset) => (
              <button className={selectedId === asset.id ? "person-asset-card active" : "person-asset-card"} type="button" key={asset.id} onClick={() => {
                setCreatingNew(false);
                setSelectedId(asset.id);
              }}>
                <span className="person-asset-thumb">
                  {asset.imageUrl ? <img src={asset.imageUrl} alt={asset.name} /> : <Image size={20} />}
                </span>
                <span className="person-asset-card-copy">
                  <strong>{asset.name}</strong>
                  <span>{personAssetTypeText(asset.personType)} · {asset.imageName || "未上传图片"}</span>
                  <em>{asset.updatedAt ? formatDate(asset.updatedAt) : "-"}</em>
                </span>
                <StatusBadge value={asset.complianceStatusText || asset.complianceStatus} tone={personAssetStatusTone(asset.complianceStatus)} />
              </button>
            )) : (
              <div className="empty-inline">暂无人物素材，先在右侧上传一张已校验的人物图。</div>
            )}
          </div>
        </div>

        <form className="panel person-asset-editor-panel" onSubmit={submitPersonAsset}>
          <div className="panel-head">
            <div>
              <h2>{selectedId === "new" ? "新增人物素材" : "编辑人物素材"}</h2>
              <p className="panel-note">人工在 libTV 完成合规后，把状态改成“已通过”，批量任务就能复用。</p>
            </div>
            <div className="person-asset-editor-actions">
              {selectedAsset && draft.complianceStatus !== "approved" ? (
                <button className="secondary-button person-asset-approve-button" type="button" onClick={() => {
                  updateDraft("complianceStatus", "approved");
                  updateDraft("complianceMethod", "manual");
                  setMessage("已切换为已通过，点击保存素材后，批量生成可以复用这张人物图。");
                }}>
                  <CheckCircle2 size={16} />
                  <span>标记已通过</span>
                </button>
              ) : null}
              <button className="primary-button person-asset-save-button" type="submit" disabled={saving}>
                <ShieldCheck size={16} />
          ...
          <div className="person-asset-editor-grid">
            <label className="field">
              <span>素材名称</span>
              <input value={draft.name} onChange={(event) => updateDraft("name", event.target.value)} placeholder="例如：女装真人模特-已通过-001" />
            </label>
            <label className="field">
              <span>素材类型</span>
              <select value={draft.personType} onChange={(event) => updateDraft("personType", event.target.value)}>
                <option value="real_person">真人图片</option>
                <option value="digital_human">数字人</option>
                <option value="model_reference">模特参考</option>
              </select>
            </label>
            <label className="field">
              <span>合规状态</span>
              <select value={draft.complianceStatus} onChange={(event) => updateDraft("complianceStatus", event.target.value)}>
                {personAssetStatusOptions.map((option) => (
            ...
            <label className="field">
              <span>校验方式</span>
              <select value={draft.complianceMethod} onChange={(event) => updateDraft("complianceMethod", event.target.value)}>
                <option value="manual">人工右键合规校验</option>
                <option value="bridge">桥接自动化</option>
                <option value="api">接口回写</option>
              </select>
            </label>
          </div>

          <div className="person-asset-media-row">
          <label className="field upload-box person-asset-upload">
            <span>人物图片</span>
            <input className="file-input-native" type="file" accept="image/*" onChange={(event) => handleImageChange(event.target.files)} />
            <span className="file-picker-shell">
              <span className="file-picker-button">
                <Upload size={16} />
                <span>上传图片</span>
              </span>
              <span className="file-picker-status">{imageFile?.name || selectedAsset?.imageName || "未选择图片"}</span>
            </span>
          </label>

          {previewImage ? (
            <div className="person-asset-preview">
              <img src={previewImage} alt={draft.name || "人物素材"} />
              <div>
                <strong>{imageFile?.name || selectedAsset?.imageName || "人物图片"}</strong>
                <span>{imageFile ? formatBytes(imageFile.size) : selectedAsset?.imageSize ? formatBytes(selectedAsset.imageSize) : "已保存"}</span>
              </div>
            </div>
          ) : null}
          </div>
          ...
          <div className="person-asset-editor-grid">
            <label className="field">
              <span>libTV 项目 ID</span>
              <input value={draft.libtvProjectId} onChange={(event) => updateDraft("libtvProjectId", event.target.value)} placeholder="可选" />
            </label>
            <label className="field">
              <span>图片节点 Key</span>
              <input value={draft.libtvImageNodeKey} onChange={(event) => updateDraft("libtvImageNodeKey", event.target.value)} placeholder="可选" />
            </label>
            <label className="field">
              <span>图片节点名称</span>
              <input value={draft.libtvImageNodeName} onChange={(event) => updateDraft("libtvImageNodeName", event.target.value)} placeholder="可选" />
            </label>
            <label className="field">
              <span>素材 ID / URI</span>
              <input value={draft.assetId || draft.assetUri} onChange={(event) => {
                updateDraft("assetId", event.target.value);
                updateDraft("assetUri", event.target.value);
              }} placeholder="可选" />
            </label>
          </div>
          ...
          <label className="field">
            <span>备注</span>
            <textarea value={draft.notes} onChange={(event) => updateDraft("notes", event.target.value)} placeholder="记录这张图适合的品类、风格、校验时间、使用限制等" />
          </label>
        </form>
      </div>
    </section>
...
function buildMobileCreateSteps({ studio, promptSteps, promptRunning, videoSteps, videoRunning, videoUrl }) {
  ...
  return [
    ...
    {
      label: "提示词包",
      detail: hasPromptPack ? "提示词包已准备" : "上传 Word 或粘贴 SOP 文本",
